...
EMAIL_ACCOUNTS_RECEIVE_ERRORS = (
    _accounts_receive_errors.split(",") if _accounts_receive_errors else []
)
...
DIMENSIONS_METRICS_URL = os.environ.get(
    "OPAC_DIMENSIONS_METRICS_URL", "https://badge.dimensions.ai/details/doi"
)
...
SCITE_URL = os.environ.get(
    "OPAC_SCITE_URL", "//cdn.scite.ai/badge/scite-badge-latest.min.js"
)
SCITE_METRICS_URL = os.environ.get(
    "OPAC_SCITE_METRICS_URL", "https://scite.ai/reports/"
)

ORCID_URL = os.environ.get("OPAC_ORCID_URL", "http://orcid.org/")
...
SSM_MEDIA_URI = "{scheme}://{domain}:{port}{path}".format(
    scheme=SSM_SCHEME, domain=SSM_DOMAIN, port=SSM_PORT, path=SSM_MEDIA_PATH
)
...
GOOGLE_VERIFY_RECAPTCHA_URL = os.environ.get(
    "OPAC_GOOGLE_VERIFY_RECAPTCHA_URL",
...
AUDIT_LOG_NOTIFICATION_ENABLED = (
    os.environ.get("OPAC_AUDIT_LOG_NOTIFICATION_ENABLED", "True") == "True"
)
_audit_log_notification_recipients = os.environ.get(
    "OPAC_AUDIT_LOG_NOTIFICATION_RECIPIENTS", None
)
AUDIT_LOG_NOTIFICATION_RECIPIENTS = (
    _audit_log_notification_recipients.split(",")
    if _audit_log_notification_recipients
    else []
)
...
FILTER_SECTION_ENABLE = (str(os.environ.get("OPAC_FILTER_SECTION_ENABLE")) == 'True')

# para uso combinado com FILTER_SECTION_ENABLE
FILTER_SECTION_ENABLE_FOR_MIN_STUDY_AREAS = int(
    os.environ.get("OPAC_FILTER_SECTION_ENABLE_FOR_MIN_STUDY_AREAS") or 10
)
...
# Common Style List
COMMON_STYLE_LIST = os.environ.get(
    "OPAC_COMMON_STYLE_LIST",
    os.path.abspath(os.path.join(PROJECT_PATH, "media/csl_styles.json")),
)
...
JWT_PUBLIC_KEY_PATH = os.environ.get("JWT_PUBLIC_KEY_PATH", default="/app/jwt_public_local.pem")
