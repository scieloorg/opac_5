...
def pull_lang(endpoint, values):
    ...
    if values is None:
        return
    lang = values.pop('ilng', None)
    supported_langs = current_app.config.get("LANGUAGES", {}).keys()
...
def add_lang(endpoint, values):
    """
    Inject language into all URLs generated by url_for.
    This ensures all URLs have the language prefix.
    """
    if 'ilng' not in values:
        if hasattr(g, 'lang'):
            values['ilng'] = g.lang
        else:
            # Fallback to default language if g.lang is not set
            values['ilng'] = current_app.config.get("BABEL_DEFAULT_LOCALE", "pt_BR")
...
def add_langs():
    """
    Add available languages to g object for use in templates.
    """
...
def add_scielo_org_config_to_g():
    """
    Set SciELO org links based on current language from URL.
    """
    language = get_locale()
    scielo_org_links = {
...
def get_locale():
    """
    Define the active language based on URL parameter.
    This is used by Flask-Babel for translations.
    """
